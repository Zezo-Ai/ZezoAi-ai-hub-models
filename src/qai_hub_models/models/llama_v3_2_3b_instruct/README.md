...
This repository contains scripts for optimized on-device export suitable to run on Qualcomm® devices. More details on model performance across various devices, can be found [here](https://aihub.qualcomm.com/models/llama_v3_2_3b_instruct).
...
```
For llama_v3_2_3b_instruct, some additional functionality can be faster or is available
...
```bash
# NOTE: 3.10 <= PYTHON_VERSION < 3.14 is supported.
pip install "qai-hub-models[llama-v3-2-3b-instruct]" onnxruntime-gpu==1.23.2 https://github.com/quic/aimet/releases/download/2.26.0/aimet_onnx-2.26.0+cu121-cp310-cp310-manylinux_2_34_x86_64.whl -f https://download.pytorch.org/whl/torch_stable.html
```
...
```bash
python -m qai_hub_models.models.llama_v3_2_3b_instruct.export --device "Samsung Galaxy S25 (Family)"
```
