...
This repository contains scripts for optimized on-device export suitable to run on Qualcomm® devices. More details on model performance across various devices, can be found [here](https://aihub.qualcomm.com/models/qwen3_8b).
...
pip install "qai-hub-models[qwen3-8b]"
```
For qwen3_8b, some additional functionality can be faster or is available
...
If you are quantizing your own variant of qwen3_8b, a dedicated CUDA enabled
...
```bash
# NOTE: 3.10 <= PYTHON_VERSION < 3.14 is supported.
pip install "qai-hub-models[qwen3-8b]" onnxruntime-gpu==1.23.2 https://github.com/quic/aimet/releases/download/2.26.0/aimet_onnx-2.26.0+cu121-cp310-cp310-manylinux_2_34_x86_64.whl -f https://download.pytorch.org/whl/torch_stable.html
```
...
```bash
python -m qai_hub_models.models.qwen3_8b.demo
```
...
```bash
python -m qai_hub_models.models.qwen3_8b.export --device "Samsung Galaxy S25 (Family)"
```
