...
This repository contains scripts for optimized on-device export suitable to run on Qualcomm® devices. More details on model performance across various devices, can be found [here](https://aihub.qualcomm.com/models/efficientvit_l2_cls).
...
```bash
python -m qai_hub_models.models.efficientvit_l2_cls.demo { --quantize w8a16, w8a16_mixed_fp16 }
```
...
```bash
python -m qai_hub_models.models.efficientvit_l2_cls.export { --quantize w8a16, w8a16_mixed_fp16 }
```
