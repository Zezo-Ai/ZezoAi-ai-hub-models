...
This repository contains scripts for optimized on-device export suitable to run on Qualcomm® devices. More details on model performance across various devices, can be found [here](https://aihub.qualcomm.com/models/gear_guard_net).
...
```bash
python -m qai_hub_models.models.gear_guard_net.demo { --quantize w8a16, w8a8 }
```
...
```bash
python -m qai_hub_models.models.gear_guard_net.export { --quantize w8a16, w8a8 }
```
