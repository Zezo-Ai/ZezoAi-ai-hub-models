...
This repository contains scripts for optimized on-device export suitable to run on Qualcomm® devices. More details on model performance across various devices, can be found [here](https://aihub.qualcomm.com/models/crestereo).
...
```bash
python -m qai_hub_models.models.crestereo.demo
```
...
```bash
python -m qai_hub_models.models.crestereo.export
```
