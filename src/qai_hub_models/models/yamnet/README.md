...
This repository contains scripts for optimized on-device export suitable to run on Qualcomm® devices. More details on model performance across various devices, can be found [here](https://aihub.qualcomm.com/models/yamnet).

Qualcomm AI Hub Models uses [Qualcomm AI Hub Workbench](https://workbench.aihub.qualcomm.com) to compile, profile, and evaluate this model. [Sign up](https://myaccount.qualcomm.com/signup) to run these models on a hosted Qualcomm® device.

## Quick Start

Use our lightweight command-line interface to inspect and download YamNet:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info YamNet

# Print performance and accuracy metrics
qai-hub-models perf YamNet
qai-hub-models numerics YamNet

# Download a ready-to-deploy asset
qai-hub-models fetch YamNet --runtime tflite --precision float
```
...
```bash
python -m qai_hub_models.models.yamnet.demo { --quantize w8a16, w8a8 }
```
...
```bash
qai-hub-models export yamnet --target-runtime tflite --precision float
```
