...
This repository contains scripts for optimized on-device export suitable to run on Qualcomm® devices. More details on model performance across various devices, can be found [here](https://aihub.qualcomm.com/models/internimage).
...
pip install "qai-hub-models[internimage]"
```
...
```bash
python -m qai_hub_models.models.internimage.export --device "Samsung Galaxy S25 (Family)" { --quantize w8a8 }
```
