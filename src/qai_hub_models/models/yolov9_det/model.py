...
DEFAULT_WEIGHTS = "yolov9t.pt"


class YoloV9Detector(Yolo):
    """Exportable YoloV9 bounding box detector, end-to-end."""
    ...
    def from_pretrained(
        ...
        if ckpt_name not in SUPPORTED_WEIGHTS:
            raise ValueError(
                f"ckpt_name must be one of {SUPPORTED_WEIGHTS}, got {ckpt_name!r}"
            )
    ...
    def forward(
        ...
        """
        Run YoloV9 on `image`, and produce a predicted set of bounding boxes and associated class probabilities.
