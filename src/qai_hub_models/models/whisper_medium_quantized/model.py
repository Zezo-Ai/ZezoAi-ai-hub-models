...
WHISPER_MEDIUM_QUANTIZED_CAPABILITIES = WhisperCapabilities(
    streaming=False,
    file_based=True,
...
class WhisperMediumEncoderQuantizable(WhisperEncoderQuantizableBase):
    @classmethod
    def make_torch_model(cls) -> HfWhisperEncoder:
        return WhisperMedium.from_pretrained().encoder
...
class WhisperMediumDecoderQuantizable(WhisperDecoderQuantizableBase):
    @classmethod
    def make_torch_model(cls) -> HfWhisperDecoder:
        return WhisperMedium.from_pretrained().decoder
...
class WhisperMediumQuantized(WorkbenchModelCollection):
    def __init__(
        self,
    ...
    def from_pretrained(cls) -> Self:
        encoder = WhisperMediumEncoderQuantizable.from_pretrained()
        decoder = WhisperMediumDecoderQuantizable.from_pretrained()
    ...
    def write_supplementary_files(
        ...
        write_whisper_supplementary_files(
            ...
            WHISPER_MEDIUM_QUANTIZED_CAPABILITIES,
            TIKTOKEN_URL,
            display_name="Whisper Medium (Quantized)",
        )
