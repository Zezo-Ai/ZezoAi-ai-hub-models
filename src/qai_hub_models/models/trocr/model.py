...
class TrOCREncoder(BaseModel):
    ...
    def forward(
        ...
        -------
        KVCache
            tuple[kv_cache_cross_attn_0_key, kv_cache_cross_attn_0_val, kv_cache_cross_attn_1_key, ...].
            KV Cache for cross attention layers. len(tuple) == 2 * number of source model decoder layers.
        """
...
class TrOCRDecoder(BaseModel):
    ...
    def __init__(
        ...
        for layer in self.decoder.model.decoder.layers:
            layer_attn: TrOCRAttention = layer.encoder_attn
            layer_attn.k_proj = None  # type: ignore[assignment, unused-ignore]
            layer_attn.v_proj = None  # type: ignore[assignment, unused-ignore]
    ...
    def forward(
        ...
        **kv_cache_kwargs: torch.Tensor,
        ...
        index
            torch.tensor, shape = (batch_size, 1), index to get the attn_mask to mask out padded kv cache.
        *kv_cache_args
            tuple[kv_cache_attn_0_key, kv_cache_attn_0_val, kv_cache_cross_attn_0_key, kv_cache_cross_attn_0_val, ...].
            Combined KV Cache generated by combine_kv_caches in app.py.
            len(tuple) == 4 * number of source model decoder layers.
        **kv_cache_kwargs
            Keyword arguments for KV cache (alternative to *kv_cache_args).
...
class TrOCR(PretrainedCollectionModel):
    def __init__(
        ...
        self.io_processor.image_processor.image_mean = [0.0, 0.0, 0.0]  # type: ignore[attr-defined, unused-ignore]
        self.io_processor.image_processor.image_std = [1.0, 1.0, 1.0]  # type: ignore[attr-defined, unused-ignore]
