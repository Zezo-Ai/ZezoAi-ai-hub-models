# [Whisper-Base: Transformer-based automatic speech recognition (ASR) model for multilingual transcription and translation available on HuggingFace](https://aihub.qualcomm.com/models/whisper_base)
...
This repository contains scripts for optimized on-device export suitable to run on Qualcomm® devices. More details on model performance across various devices, can be found [here](https://aihub.qualcomm.com/models/whisper_base).

Qualcomm AI Hub Models uses [Qualcomm AI Hub Workbench](https://workbench.aihub.qualcomm.com) to compile, profile, and evaluate this model. [Sign up](https://myaccount.qualcomm.com/signup) to run these models on a hosted Qualcomm® device.

## Quick Start

Use our lightweight command-line interface to inspect and download Whisper-Base:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info Whisper-Base

# Print performance and accuracy metrics
qai-hub-models perf Whisper-Base
qai-hub-models numerics Whisper-Base

# Download a ready-to-deploy asset
qai-hub-models fetch Whisper-Base --runtime qnn_context_binary --precision float
```
...
```bash
qai-hub-models export whisper_base --target-runtime qnn_context_binary --precision float --device "Samsung Galaxy S25 (Family)"
```
