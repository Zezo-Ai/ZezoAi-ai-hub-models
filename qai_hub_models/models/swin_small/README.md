...
This repository contains scripts for optimized on-device export suitable to run on Qualcomm® devices. More details on model performance across various devices, can be found [here](https://aihub.qualcomm.com/models/swin_small).
...
```bash
python -m qai_hub_models.models.swin_small.demo { --quantize w8a16 }
```
...
```bash
python -m qai_hub_models.models.swin_small.export { --quantize w8a16 }
```
